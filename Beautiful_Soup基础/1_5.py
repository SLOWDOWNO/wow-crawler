...
def get_content(url):
    ...
    li_tags = soup.find_all(
        "li", attrs={"class": ["j_thread_list clearfix thread_item_box"]}
    )
    ...
    for li in li_tags:
        ...
        try:
            ...
            comment["name"] = li.find(
                "a", attrs={"class": ["frs-author-name j_user_card"]}
            ).string
...
def out_to_file(content):
    ...
    """
    file_path = Path("data/txt") / "生活大爆炸吧帖子信息.txt"
    with open(file_path, "a+", encoding="utf-8") as file:
        for comment in content:
            file.write(
                f"标题：{comment['title']}\t链接：{comment['link']}\t发帖ID: {comment['name']}\n"
            )
